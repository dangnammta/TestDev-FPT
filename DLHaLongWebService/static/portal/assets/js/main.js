...
var mapHandler = {
    ...
    init: function () {
        ...
        var mapSelector = document.getElementById('map'),
                mapLat = mapSelector.dataset.lat || 20.9701263, //21.0287797
                mapLng = mapSelector.dataset.lng || 107.0438938, //105.850176
                mapZoom = mapSelector.dataset.zoom || 14;
    ...
    addTabsEvent: function () {
        $('.targets-tab a').click(function (e) {
            ...
            $.ajax({
                ...
                $.each(pinList, function (index, el) {
                    var tpl_content = '<p><strong>' + el.title + '</strong>';
                    tpl_content += '<div class="rate view">' +
                            ...
                            ' <a href="#">♥</a>' +
                            ' </div><br>';
                    tpl_content += '<p>' + el.content + '</p>';
                    tpl_content += '<a href="' + el.link + '" class="text-uppercase btn btn-outline-secondary">Xem chi tiết</a>';
...
//                            });
                    ...
                    var pinMarker = new google.maps.Marker({
                        ...
                        icon: pinImg,
                        title: el.title,
                        description: tpl_content,
                        animation: google.maps.Animation.DROP
                    });
                    pinMarker.addListener('click', function () {

                        infowindow.close();//hide the infowindow
                        infowindow.setContent(pinMarker.description);//update the content for this marker
                        infowindow.open(mapHandler.map, pinMarker);//"move" the info window to the clicked marker and open it
                    });
